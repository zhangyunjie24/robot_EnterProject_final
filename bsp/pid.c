...
float pid_calc(pid_t* pid, float get, float set){
    ...
    if (pid->max_err != 0 && ABS(pid->err[NOW]) >  pid->max_err  )  //过大的误差需要异常处理
		return 0;
	if (pid->deadband != 0 && ABS(pid->err[NOW]) < pid->deadband)   //过小的误差进入死区，不进行调参
		return 0;
    ...
    if(pid->pid_mode == POSITION_PID) //位置式p
    {
        ...
        pid->pos_out = pid->pout + pid->iout + pid->dout;   //三者相加
